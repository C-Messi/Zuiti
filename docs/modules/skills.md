...
Skills 模块现在只沉淀可挂载的 SVG 道具 skill。道具 skill 归属于 active pet 包，锚点也从该宠物包读取；宠物本体动作由宠物包 motion tools 负责，避免 LLM 每次重新绘制出不同的宠物。
...
仓库保留 `pet_resources/pets/default-cat/skills/example-soft-wave/` 作为示范道具。真实运行时自动生成的 skill 默认被 `.gitignore` 排除。旧的整宠 `action.svg` 包不会被加载。
...
6. 审核分达到阈值后写入 `pet_resources/pets/<pet_id>/skills/<skill_id>/` 并自动启用。
