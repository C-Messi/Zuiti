...
import { buildTextSystemPromptWithMotion, buildTextUserPrompt, type PromptContext } from './prompt'
...
export async function respond(ctx: PromptContext, userText?: string): Promise<BrainResponse> {
  ...
  const system = buildTextSystemPromptWithMotion(
    snapshot.soulText,
    ctx.tone,
    snapshot.motionPromptText
  )
